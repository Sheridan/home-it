...
xrandr --verbose \
    ...
    {% for monitor_name in merged_vars.Xorg.options.init_order -%}
      {%- set monitor = merged_vars.Xorg.monitors[monitor_name] -%}
    --output "{{ monitor.video_out }}" --mode {{ monitor.mode.width }}x{{ monitor.mode.height }} --rate {{ monitor.mode.rate }} {% if 'base' not in monitor.position %} --{{ monitor.position.where }} "{{ merged_vars.Xorg.monitors[monitor.position.from].video_out }}" {% else %} --primary {% endif %}{% if 'rotate' in monitor %}--rotate {{ monitor.rotate }}{% endif %} \
    {# -#}
    {%- endfor %}
